 ...
 */
/* The `formatPhoneNumber` function is designed to format a phone number as '+998 99 777 66 55'. It
takes a phone number as a parameter, checks if the phone number is either 9 or 12 characters long,
and then formats it accordingly. If the phone number is 9 characters long, it adds the country code
and formats it with spaces. If the phone number is 12 characters long, it assumes it already has the
country code and formats it with spaces. If the phone number is not 9 or 12 characters long, it
throws an error indicating that the phone number must be either 9 or 12 characters long. */
function formatPhoneNumber(phoneNumber) {
  ...
  if (phoneNum.length === 9) {
    return `+998 ${phoneNum.slice(0, 2)} ${phoneNum.slice(
      2,
      5
    )} ${phoneNum.slice(5, 7)} ${phoneNum.slice(7)}`;
  } else if (phoneNum.length === 12) {
    return `+${phoneNum.slice(0, 3)} ${phoneNum.slice(3, 5)} ${phoneNum.slice(
      5,
      8
    )} ${phoneNum.slice(8, 10)} ${phoneNum.slice(10)}`;
  } else {
    throw new Error("Phone number must be either 9 or 12 characters long");
  }
}
...
function replaceWordInText(text, word, replacement) {
  const updatedText = text.split(/\b/).map((txt) => {
...
function extractPriceFromText(text) {
  ...
  return match ? +match[1] : "No matching price was found";
}
